...
| File | Holds |
| --- | --- |
| `profile.json` | Name, role, institution, email, CV link, home headline and lede, about text, areas, education, contact rows, social links |
| `publications.json` | One array. `"type": "book"` or `"type": "article"` decides which heading an entry renders under; `pdf` is optional and the link is omitted when absent |
| `navigation.json` | The five nav items, in order |
| `site.json` | Site-level only — url, language, meta description, browser theme colors. Deliberately has no `title`: the name comes from `profile.json` so there is one place to change it |

`profile.json` accepts inline HTML in `home.headline` and `about.body` — that is how the
...
- The real URL in `site.json` (currently `https://example.com`).
